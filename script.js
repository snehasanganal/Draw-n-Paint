...
const setCanvasBackground = () => {
    // setting whole canvas background to white , sothe downloaded img background will be white
    ctx.fillStyle ="#fff";
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = selectedColor; // setting back to the selected color, it'll be the brush color
}


window.addEventListener("load", () => {
    //setting canvas width/height.. offsetwidth/height returns viewalbe width/height of an element
...
const drawRect = (e) => {
    //if fill color is not checked draw a rect with border else draw rect with background
    if(!fillColor.checked){
        //creating rect according to mouse pointer
...
const drawTriangle =  (e) => {
    ...
    ctx.closePath(); //closing triangle with third line automatically
...
const drawLine = (e) => {
    ...
    ctx.moveTo(prevMouseX, prevMouseY); // moving line to mouse pointer 
    ctx.lineTo(e.offsetX, e.offsetY); //creating first line according to mouse pointer
    ctx.stroke();
}
...
const startDraw = (e) => {
    ...
    ctx.lineWidth = brushWidth; //passing brushsize s line width
...
saveImg.addEventListener("click", () =>{
    ...
    link.href = canvas.toDataURL(); // passing canvas Data link href value
